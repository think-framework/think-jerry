...
public class ServletRequestAdapter implements HttpServletRequest {
    ...
    public ServletRequestAdapter(Request request) {
        ...
        request.getHeaders().forEach((headerName,headerValue) -> {
            headers.put(headerName, headerValue.stream().collect(Collectors.joining(",")));
        });
    }
    ...
    public <T extends HttpUpgradeHandler> T upgrade(Class<T> handlerClass) throws IOException, ServletException {
        try {
            // TODO 优化这的反射
            this.httpUpgradeHandler = handlerClass.newInstance();
            request.getProcessor().setUpgradeToken(new UpgradeToken(httpUpgradeHandler));
    ...
    public ServletInputStream getInputStream() throws IOException {
        ...
        return new ServletInputStream() {
            ...
            public int read() throws IOException {
                return bytes[i.getAndIncrement()];
            }

            @Override
            public byte[] readAllBytes() throws IOException {
                return super.readNBytes(bytes.length);
            }
        };
    }
